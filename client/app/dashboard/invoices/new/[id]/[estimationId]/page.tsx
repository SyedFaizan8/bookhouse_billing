...
export default function EstimationPage() {
    const { id, estimationId } = useParams<{ id: string, estimationId: string }>();
    const router = useRouter();

    const { data: customer, isLoading } = useSchoolProfile(id);
    const { data: company } = useSettingsInfo();
    const { data: user } = useAuthUser();
    const { data: estimationData, isLoading: estimationLoading } = useInvoicePdf(estimationId)
    const { data: invoiceNumberData } = useNextInvoiceNumber();
    ...
    const submit = (data: Form) => {
        ...
        for (const [i, item] of data.items.entries()) {
            ...
            if (item.discountPercent >= 100) {
                toast.warning(`Item ${i + 1}: Discount cannot be 100%`);
                return;
            }
        ...
        if (totals.finalAmount <= 0) {
            toast.warning("Invoice total must be greater than zero");
            return;
        }
    ...
    useEffect(() => {
        ...
        form.reset({
            items: estimationData.items.map((item) => ({
    ...
    if (isLoading || !customer || !company || estimationLoading) return null;
    ...
    return (
        <div className="py-4">
            ...
            {/* BACK */}
            <button
                onClick={() => router.push(`/dashboard/schools/${id}/estimations`)}
            ...
            <div className="mx-auto bg-white shadow-2xl p-8 rounded-lg space-y-6">
                ...
                <div className="text-center">
                    <div className="flex justify-between text-sm text-slate-600">
                        {invoiceNumberData && (
                            <span>Invoice #{invoiceNumberData.nextNumber}</span>
                        )}
                ...
                <button
                    ...
                    className="w-full bg-indigo-700 hover:bg-indigo-800 text-white py-3 rounded-lg text-lg disabled:opacity-50 disabled:cursor-not-allowed"
                >
