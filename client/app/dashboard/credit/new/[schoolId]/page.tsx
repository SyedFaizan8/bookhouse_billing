...
import { useNextCreditNumber } from "@/lib/queries/nextNumber";
...
export default function CreditNotePage() {
    ...
    const { data: customer, isLoading } = useSchoolProfile(schoolId);
    const { data: company } = useSettingsInfo();
    const { data: user } = useAuthUser();
    const { data: invoiceNumberData } = useNextCreditNumber();

    const createInvoice = useCreateCreditNote();
    ...
    const submit = (data: Form) => {
        ...
        if (totals.finalAmount <= 0) {
            toast.warning("Credit Note total must be greater than zero");
            return;
        }
